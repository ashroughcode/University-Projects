...
void iDraw()
{
	...
	if (flag == 1)
	{
		...
		iShowBMP2(780, 20, "skip_btn.bmp", 0);	
	}
	...
	if (flag == 2)
	{
		...
		for (int i = 0; i<3; i++)
		{
			fscanf(fptr, "%d %s", &scoreshow[i].scorearray, &scoreshow[i].names);

		}

		//fscanf(fptr,"%d %s",&hscore,&name);
		sprintf(iscore1, "%d", scoreshow[0].scorearray);
		sprintf(iscore2, "%d", scoreshow[1].scorearray);
		sprintf(iscore3, "%d", scoreshow[2].scorearray);
		fclose(fptr);

		iSetColor(255, 0, 0);
		iText(588, 398, scoreshow[0].names, GLUT_BITMAP_TIMES_ROMAN_24);
		iSetColor(255, 0, 0);
		iText(690, 398, iscore1, GLUT_BITMAP_TIMES_ROMAN_24);

		iSetColor(250, 181, 5);
		iRectangle(580, 330, 300, 30);
		iSetColor(255, 0, 0);
		iText(588, 338, scoreshow[1].names, GLUT_BITMAP_TIMES_ROMAN_24);
		iSetColor(255, 0, 0);
		iText(690, 338, iscore2, GLUT_BITMAP_TIMES_ROMAN_24);

		iSetColor(250, 181, 5);
		iRectangle(580, 270, 300, 30);
		iSetColor(255, 0, 0);
		iText(588, 278, scoreshow[2].names, GLUT_BITMAP_TIMES_ROMAN_24);
		iSetColor(255, 0, 0);
		iText(690, 278, iscore3, GLUT_BITMAP_TIMES_ROMAN_24);
	...
	if (flag == 17)
	{
		...
		iShowBMP2(770, 30, "next_btn.bmp", 0);
	}
	...
	if (pauseMenu)
	{
		...
		iShowBMP2(400, 210, "exit_btn.bmp", 0);
	}
...
void iMouse(int button, int state, int mx, int my)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		if (flag == 17 && mode == 0 && mx >= 400 && mx <= 650 && my >= 300 && my <= 350)
		{
			mode = 1;
		}
		...
		if (flag == 1 && mx >= 786 && mx <= 974 && my >= 27 && my <= 72)
		{
			flag = 14;
		}
		...
		if (flag == 0 && mx >= 407 && mx <= 593 && my >= 198 && my <= 243)
		{
			flag = 3;
			//flagIn = 0;
		}
		if (flag == 3 && mx >= 787 && mx <= 973 && my >= 27 && my <= 73)
		{
			flag = 0;
		}
		/*
		...
		*/
		...
		if (flag == 4 && mx >= 787 && mx <= 973 && my >= 27 && my <= 73)
		{
			flag = 0;
		}
...
void updateScore()
{
	...
	scoreshow[3].scorearray = totalScore;
	strcpy(scoreshow[3].names, str2);
	for (int i = 0; i<4; i++)
	{
		for (int j = i + 1; j<4; j++)
		{
			if (scoreshow[i].scorearray < scoreshow[j].scorearray)
			{
				tp = scoreshow[j].scorearray;
				scoreshow[j].scorearray = scoreshow[i].scorearray;
				scoreshow[i].scorearray = tp;

				strcpy(tmp, scoreshow[j].names);
				strcpy(scoreshow[j].names, scoreshow[i].names);
				strcpy(scoreshow[i].names, tmp);
	...
	for (int i = 0; i<3; i++)
	{
		fprintf(fptr, "%d %s \n", scoreshow[i].scorearray, scoreshow[i].names);
